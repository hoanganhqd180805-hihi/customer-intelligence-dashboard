  ...
  return (
    <>
      {Array.from({ length: 6 }, (_, index) => (
        <Card
          key={index}
          className={`flex h-[109px] flex-col justify-center px-4 py-[17px] ${status === "loading" ? "animate-pulse" : ""}`}
        >
          <span className="text-[13px] text-[#707070]">{label}</span>
          {index === 0 && message ? (
            <span
              className="mt-2 truncate text-[11px] text-[#3b82f6]"
              title={message}
            >
              {message}
            </span>
          ) : (
            <span className="mt-3 h-4 w-20 rounded bg-[#eef4fd]" />
          )}
        </Card>
      ))}
    </>
...
function OverviewLoader({
  ...
  useEffect(() => {
    ...
    getOverview({ startDate, endDate, signal: controller.signal })
      .then((result) =>
        setState(
