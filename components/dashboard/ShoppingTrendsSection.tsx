...
export function ShoppingTrendsSection() {
  ...
  const selectedProducts = useMemo(() => products.filter((item) => item.productType === productType).slice(0,5),[productType]);
    ...
    <section>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card className="flex h-[337px] flex-col p-4">
          ...
          <AnimatePresence mode="wait" initial={false}><motion.ol key={productType} initial={{opacity:0,y:4}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-4}} transition={{duration:.18}} className="mt-4">{selectedProducts.map((item) => <li key={`${item.productType}-${item.rank}-${item.itemId}`} className="grid h-[43px] grid-cols-[28px_1fr_35px] items-center border-b border-[#e5e5e5] text-[11px] last:border-0"><span className="text-[#777]">{String(item.rank).padStart(2,"0")}</span><span className="truncate pr-2">{item.itemName}</span><strong className="text-right text-[#3b82f6]">{item.totalQuantitySold}</strong></li>)}</motion.ol></AnimatePresence>
        </Card>
      </div>
