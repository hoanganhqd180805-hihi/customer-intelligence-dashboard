...
const sourceNode=(label:string)=>journeyNodes.find((node)=>node.label===label)!;
const sourceLink=(source:string,target:string)=>journeyLinks.find((link)=>link.source===sourceNode(source).id&&link.target===sourceNode(target).id)!;
const linkLabel=(source:string,target:string)=>sourceLink(source,target).label;
const dropOffLabel=(source:string,target:string)=>`↓${(100-Number.parseFloat(linkLabel(source,target))).toFixed(1)}%`;
const conversionRates=[[linkLabel("Ads","Product View"),"Ads → Product View"],[linkLabel("Product View","Order"),"Product View → Order"],[linkLabel("Order","Complete"),"Order → Complete"],[linkLabel("Complete","Good Review"),"Complete → Good Review"]];
const dropoffs=[[dropOffLabel("Ads","Product View"),"Ads → Product View"],[dropOffLabel("Product View","Order"),"Product View → Order"],[dropOffLabel("Order","Complete"),"Order → Complete"]];
    ...
    <section className="rounded-[24px] border border-[#1c2350] bg-[radial-gradient(1200px_500px_at_15%_-10%,rgba(74,153,210,.18),transparent_60%),radial-gradient(1000px_480px_at_90%_110%,rgba(134,234,233,.12),transparent_60%),linear-gradient(180deg,#0a0f2b,#050714)] p-5 text-[#eef1fb] shadow-[0_22px_60px_rgba(9,14,42,.18)]">
      ...
      <div className="relative mt-1.5 overflow-hidden rounded-[18px]">
        ...
        <svg viewBox={`0 0 ${layout.width} ${layout.height}`} preserveAspectRatio="xMidYMid meet" aria-label="Customer journey chart" className="block w-full touch-manipulation" onClick={()=>setSelectedId(null)}>
          ...
          {journeyStages.map((stage,index)=>{const x=150+((1820-150-145)*index)/(journeyStages.length-1);return <g key={stage}><text x={x} y="16" textAnchor="middle" fontSize="11.5" fontWeight="700" letterSpacing="1.5" fill="#99a7cf">{stage}</text><line x1={x-34} y1="23" x2={x+34} y2="23" stroke="#303966"/></g>})}
      ...
      <div className="mt-2.5 rounded-[14px] border border-[#86eae9]/25 bg-[#86eae9]/[.08] p-3 text-[12px] leading-[1.55] text-[#cfe9ff]"><div className="grid grid-cols-1 gap-3 lg:grid-cols-[1.2fr_1fr_1fr]"><div><p className="mb-0.5 text-[10px] tracking-[.08em] text-[#7d8aa8]">BIGGEST DROP-OFF</p><b className="text-[#86eae9]">Ads → Product View · {dropOffLabel("Ads","Product View").slice(1)}</b><br/>{numberFormat.format(sourceLink("Ads","Product View").value)} product views came from Ads, with the workbook Rate preserved as {linkLabel("Ads","Product View")}.</div><div className="lg:border-l lg:border-white/[.08] lg:pl-3"><p className="mb-0.5 text-[10px] tracking-[.08em] text-[#7d8aa8]">ORDER QUALITY</p><b className="text-[#86eae9]">{numberFormat.format(completeValue)} / {numberFormat.format(orderValue)} completed orders</b><br/>The workbook Rate is {linkLabel("Order","Complete")} for Complete, {linkLabel("Order","Cancel")} for Cancel, and {linkLabel("Order","Processing")} for Processing.</div><div className="lg:border-l lg:border-white/[.08] lg:pl-3"><p className="mb-0.5 text-[10px] tracking-[.08em] text-[#7d8aa8]">POST-PURCHASE SIGNAL</p><b className="text-[#86eae9]">{numberFormat.format(goodReviewValue)} Good Review · {numberFormat.format(buyAgainValue)} Buy Again</b><br/>Workbook Rates are {linkLabel("Complete","Good Review")} for Good Review and {linkLabel("Complete","Buy Again")} for Buy Again.</div></div></div>
