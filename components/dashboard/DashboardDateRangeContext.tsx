...
interface DashboardDateRangeValue extends DashboardDateRange {
  setDateRange: (range: DashboardDateRange) => void;
}
const DashboardDateRangeContext = createContext<DashboardDateRangeValue | null>(
  null,
);

export function DashboardDateRangeProvider({ children }: PropsWithChildren) {
  const [range, setDateRange] = useState<DashboardDateRange>({
...
export function useDashboardDateRange() {
  ...
  if (!value)
    throw new Error(
      "useDashboardDateRange must be used inside DashboardDateRangeProvider",
    );
  return value;
}
