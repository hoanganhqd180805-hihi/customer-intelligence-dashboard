import type { CustomerType, ProductType, ShoppingType, Weekday } from "./dashboard";

export interface RawOverviewResponse {
  total_customers: number; total_orders: number; revenue: number; average_order_value: number;
...
export interface RawPurchaseSlotRow { weekday: Weekday; slot: string; total_orders: number; total_revenue: number | null }
...
export interface RawShoppingCompositionRow { type: ShoppingType; order_count: number; order_share_pct: number; revenue: number; revenue_share_pct: number }
export interface RawProductRow { rank: number; product_type: ProductType; item_id: string; item_name: string; total_quantity_sold: number; total_orders: number; product_sales: number }
...
export interface RawDashboardResponse {
  ...
  customer_types: { available: boolean; daily: RawCustomerTypeDailyRow[]; revenue_contribution: RawCustomerRevenueRow[] };
  purchase_cancellation: { time_slots: string[]; weekdays: Weekday[]; time_slot_totals: RawPurchaseSlotRow[]; weekday_totals: RawWeekdayOrderRow[]; cancellation_reasons: RawCancellationReasonRow[]; cancellation_lost_revenue: number };
  shopping: { composition: RawShoppingCompositionRow[]; products: RawProductRow[]; product_pairs: RawProductPairRow[] };
  journey: { stages: string[]; nodes: RawJourneyNodeRow[]; links: RawJourneyLinkRow[] };
